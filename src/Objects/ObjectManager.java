...
public class ObjectManager {
	...
	public void applyEffectToPlayer(Diamon p) {
		...
		for (GameContainer gc : containers)
			if (gc.isActive()) {
				if (gc.getHitbox().intersects(attackbox)) {
					...
					diamons.add(new Diamon((int) (gc.getHitbox().x + gc.getHitbox().width / 2), (int) (gc.getHitbox().y - gc.getHitbox().height / 2), 1));
					return;
				}
			}
	}
	...
	private void loadImgs() {
		BufferedImage potionSprite = LoadSave.GetSpriteAtlas(LoadSave.DIAMON);
		diamonImgs = new BufferedImage[10];

		for (int i = 0; i < diamonImgs.length; i++)
			diamonImgs[i] = potionSprite.getSubimage(18 * i, 0, 18, 14);
